...
/* get length of label counter string */
int lcount_length(int lcount)
{
    int length = 0;
    while(lcount % 10 > 0){
        length++;
    ...
    switch ( root->type.index )
    {
        ...
        case IF_STATEMENT:
            if(root->n_children == 2){
                ...
                generate(stream, root->children[0]);
                /* compare the expression to zero, and jump if its equal */
                instruction_add(POP, eax, NULL, 0, 0);
                instruction_add(CMP, STRDUP("$0"), eax, 0, 0);
                ...
                instruction_add(POP, eax, NULL, 0, 0);
                instruction_add(CMP, STRDUP("$0"), eax, 0, 0);
                instruction_add(JUMPEQ, STRDUP(elselabel), NULL, 0, 0);
                generate(stream, root->children[1]);
                instruction_add(JUMP, STRDUP(filabel), NULL, 0, 0);
                instruction_add(STRING, STRDUP(elselabel_colon), NULL, 0, 0);
